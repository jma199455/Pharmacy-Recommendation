...
class FormControllerTest extends Specification {
    ...
    private PharmacyRecommendationService pharmacyRecommendationService = Mock() // 약국 추천 서비스 모킹
    ...
    def "POST / search"() {
        ...
        1 * pharmacyRecommendationService.recommendPharmacyList(argument -> {
            assert argument == inputAddress // mock 객체의 argument(파리미터) 검증
        }) >> outputDtoList

        // 검증
        // attributeExists(String name) : name에 해당하는 데이터가 model에 있는지 검증
        // attribute(String name, Object value) : name에 해당하는 데이터가 vlaue 객체인지 검증
        resultActions
